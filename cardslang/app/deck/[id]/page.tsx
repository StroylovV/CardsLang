...
import { Trash2, ChevronLeft, Plus, GraduationCap, CheckCircle2, Sun, Moon } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 pb-32 font-sans transition-colors duration-300">
      <div className="max-w-2xl mx-auto px-4 pt-8">
        <header className="flex justify-between items-center mb-8 bg-white dark:bg-slate-900 p-4 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800">
          ...
          <h1 className="text-xl font-black text-gray-800 dark:text-gray-100 uppercase tracking-tight">Словарь</h1>
          
          <div className="flex gap-2">
            {mounted && (
              <button 
                onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
                className="p-2.5 rounded-xl bg-gray-50 dark:bg-slate-800 text-gray-500 dark:text-yellow-400"
              >
              ...
              <div
                ...
                className={`group relative p-6 rounded-[2rem] border-2 transition-all duration-300 ${
                  selectedWords.includes(word.id)
                    ...
                    : "bg-white dark:bg-slate-900 border-transparent dark:border-slate-800 shadow-sm hover:shadow-md hover:border-indigo-100 dark:hover:border-indigo-900"
                }`}
              >
      ...
      {selectedWords.length > 0 && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-[90%] max-w-lg bg-gray-900/95 dark:bg-slate-900/95 backdrop-blur-md text-white p-4 rounded-[2.5rem] shadow-2xl flex items-center justify-between z-50 animate-in slide-in-from-bottom-10 border border-white/5">
          <div className="pl-4">
